...
def render_fechamento_diario_simplificado(spreadsheet):
    ...
    SHEET_FECH = "Fechamento_Diario_Caixa_Interno"
    HEADERS_FECH = [
    ...
    def _load_ops_do_dia(data_alvo):
        raw = buscar_dados(spreadsheet, "Operacoes_Caixa") or []
        if not raw:
            return pd.DataFrame()

        try:
            df = pd.DataFrame(normalizar_dados_inteligente(raw))
        except Exception:
            df = pd.DataFrame(raw)

        # normaliza datas e números (aceita ambos os formatos)
        ...
        # algumas versões antigas não tinham Valor_Liquido
        if "Valor_Liquido" not in df.columns and "Valor_Bruto" in df.columns:
            df["Valor_Liquido"] = df["Valor_Bruto"]

        return df[df["Data"].eq(pd.to_datetime(data_alvo).date())].copy()

    def _calc_totais(data_alvo, saldo_anterior):
        df_dia = _load_ops_do_dia(data_alvo)

        if df_dia.empty:
            ts, tc, sup = 0.0, 0.0, 0.0
        else:
            ts  = float(df_dia.loc[df_dia["Tipo_Operacao"].isin(TIPOS_SAQUE),  "Valor_Liquido"].sum())
            tc  = float(df_dia.loc[df_dia["Tipo_Operacao"].isin(TIPOS_CHEQUE), "Valor_Liquido"].sum())
            sup = float(df_dia.loc[df_dia["Tipo_Operacao"].eq("Suprimento"),   "Valor_Bruto"].sum())

        saldo_calc = float(saldo_anterior + sup - (ts + tc))
        return ts, tc, sup, saldo_calc, df_dia
    ...
    # ---------------- 2) saldo do dia anterior ----------------
    saldo_dia_anterior, usou_zero = 0.0, True
    try:
        dados_fech = buscar_dados(spreadsheet, SHEET_FECH) or []
        df_fech = pd.DataFrame(dados_fech)
    except Exception:
        df_fech = pd.DataFrame()

    if not df_fech.empty and "Data_Fechamento" in df_fech.columns:
        df_fech["Data_Fechamento"] = pd.to_datetime(df_fech["Data_Fechamento"], errors="coerce").dt.date
        df_fech["Saldo_Calculado_Dia"] = _to_float_series(df_fech.get("Saldo_Calculado_Dia", 0))
    ...
    # ---------------- 3) calcular totais (preview) ----------------
    total_saques_cartao, total_trocas_cheque, total_suprimentos, saldo_calculado_dia, df_preview = \
        _calc_totais(data_alvo, saldo_dia_anterior)

    # alerta se tem mov sem suprimento
    if not df_preview.empty and df_preview["Tipo_Operacao"].eq("Suprimento").sum() == 0:
        st.markdown(
    ...
    with st.form("form_fechamento_caixa_simplificado", clear_on_submit=True):
        ...
        # diferença sobre o preview atual
        diferenca_preview = float(dinheiro_contado - saldo_calculado_dia)
        st.markdown(f"**Diferença (prévia):** R$ {diferenca_preview:,.2f}")

        # checa se já existe lançamento nessa data
        ...
        if not df_fech.empty:
            ...
            if not df_exist.empty:
                ...
                row_alvo = int(df_exist.index[0]) + 2  # +2 por causa do header
    ...
    # ---------------- 6) persistência (recalcula na hora) ----------------
    if btn_salvar:
        try:
            if "nome_usuario" not in st.session_state:
                st.session_state.nome_usuario = "OPERADOR"

            # **Recalcula agora** para evitar qualquer chance de 0/valores desatualizados
            ts, tc, sup, saldo_calc, _ = _calc_totais(data_alvo, saldo_dia_anterior)
            diferenca = float(dinheiro_contado) - float(saldo_calc)

            ws = get_or_create_worksheet(spreadsheet, SHEET_FECH, HEADERS_FECH)
            linha = [
                str(data_alvo),
                st.session_state.nome_usuario,
                float(saldo_dia_anterior),
                float(ts), float(tc), float(sup),
                float(saldo_calc),
    ...
    with st.expander("🔍 Conferência rápida dessa data"):
        if df_preview.empty:
            st.write("Sem operações encontradas para esta data.")
        else:
            st.dataframe(
                df_preview.groupby("Tipo_Operacao", as_index=False)[
                    ["Valor_Bruto","Taxa_Cliente","Taxa_Banco","Valor_Liquido","Lucro"]
