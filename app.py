...
def conectar_google_sheets():
    try:
        ...
        # Primeiro tenta usar Streamlit Secrets (para deploy na nuvem)
        try:
            credentials_dict = {
            ...
            # Se não encontrar secrets, tenta arquivo local
            try:
                creds = ServiceAccountCredentials.from_json_keyfile_name("credentials.json", scope)
                st.success("💻 Conectado via arquivo local")
            except FileNotFoundError:
                st.error("❌ Arquivo credentials.json não encontrado no ambiente local")
                st.info("💡 Para usar localmente, coloque o arquivo credentials.json na pasta do projeto")
                return None
        ...
        st.error("- O arquivo credentials.json está na pasta (ambiente local)")
        st.error("- Os secrets estão configurados (Streamlit Cloud)")
        st.error("- A planilha 'Lotericabasededados' existe e está compartilhada")
        return None
