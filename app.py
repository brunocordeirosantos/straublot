                    ...
                    try:
                        if "simulacao_atual" not in st.session_state:
                            ...
                            nova_operacao = [
                                ...
                                sim_data["nome"],
                                sim_data["cpf"],
                                str(sim_data["valor_bruto"]),
                                float(str(sim_data["dados"]["taxa_cliente"])),
                                float(str(sim_data["dados"]["taxa_banco"])),
                                float(str(sim_data["dados"]["valor_liquido"])),
                                float(str(sim_data["dados"]["lucro"])),
                                "Concluído",
                                "",
                                f"{(str(sim_data["dados"]["taxa_cliente"])/str(sim_data["valor_bruto"]))*100:.2f}%",
                                sim_data["observacoes"]
                            ]
                            
                            worksheet.append_row(nova_operacao)
                            st.success(f"✅ {sim_data["tipo"]} de R$ {str(sim_data["valor_bruto"]):,.2f} registrado com sucesso!")
