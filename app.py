...
def init_google_sheets():
    """Inicializa conexão com Google Sheets com fallback robusto"""
    try:
...
# ---------------------------
# Funções de Cálculo (CORRIGIDAS) - Retornam float para evitar problemas de serialização
# ---------------------------
...
# ---------------------------
# Módulos de Renderização - CONTINUAÇÃO
# ---------------------------
...
def render_form_saque_cartao(spreadsheet, tipo_cartao):
    ...
    # CORREÇÃO: Botão de simulação com cálculos corretos
    if st.button("🧮 Simular Operação", use_container_width=True, key=f"simular_saque_{tipo_cartao}"):
        if valor > 0:
            ...
            # Armazenar simulação corretamente
    ...
    if st.button("🧮 Simular Operação", use_container_width=True, key=f"simular_ch_{tipo_cheque}"):
        if valor > 0:
            ...
            if tipo_cheque == "Cheque à Vista":
                ...
                if calc: taxa_percentual_str = f"{(calc['taxa_total'] / valor) * 100:.2f}%"
